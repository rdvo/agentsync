...
config_get_targets_array() {
    ...
    # Helper to push current target
    push_target() {
        if [[ -n "$current_tool" ]] && [[ -n "$current_path" ]]; then
            targets+=("${current_tool}:${current_path}:${current_enabled:-true}")
        fi
        current_tool=""
        current_path=""
        current_enabled=""
    ...
    while IFS= read -r line; do
        if [[ -n "$line" ]]; then
            ...
            if [[ "$line" =~ enabled:[[:space:]]*([^[:space:]]*) ]]; then
...
config_write() {
    local source="$1"
    local global_source="$2"
    local watch_enabled="$3"
    local git_hooks_enabled="$4"
    shift 4
    local targets=("$@")
